...
FIXES vs v1:
  ...
  • use_container_width=True → width='stretch'       [Streamlit deprecation]
  • st.components.v1.html → st.components.v1.html   [kept: st.iframe can't run JS]
  • Cloud persistence: weights/params survive Streamlit Cloud restarts via secrets
  • cat_now variable was unused — removed
  • Countdown spam: cached per game_id, renders once per game
"""
...
@keyframes headerPulse  { 0%,100%{background-position:0% 50%} 50%{background-position:100% 50%} }
@keyframes slideInUp    { from{transform:translateY(24px);opacity:0} to{transform:translateY(0);opacity:1} }
@keyframes slideInLeft  { from{transform:translateX(-20px);opacity:0} to{transform:translateX(0);opacity:1} }
@keyframes glowPulse    { 0%,100%{box-shadow:0 0 8px var(--clutch-green)} 50%{box-shadow:0 0 24px var(--clutch-green),0 0 48px rgba(0,230,118,0.3)} }
@keyframes redGlow      { 0%,100%{box-shadow:0 0 8px var(--clutch-red)} 50%{box-shadow:0 0 24px var(--clutch-red),0 0 48px rgba(255,23,68,0.3)} }
@keyframes livePulse    { 0%,100%{opacity:1;transform:scale(1)} 50%{opacity:0.6;transform:scale(1.05)} }
@keyframes fadeIn       { from{opacity:0} to{opacity:1} }
@keyframes shimmer      { 0%{background-position:-200% center} 100%{background-position:200% center} }
@keyframes borderFlow   { 0%{border-color:var(--clutch-orange)} 33%{border-color:var(--clutch-gold)} 66%{border-color:var(--clutch-blue)} 100%{border-color:var(--clutch-orange)} }

.clutch-hero {
  background: linear-gradient(135deg,#0A0A0F 0%,#1a0a00 30%,#0d0d1a 60%,#0A0A0F 100%);
  background-size: 300% 300%;
  animation: headerPulse 8s ease infinite;
  border-radius: 16px; padding: 32px 40px 24px; margin-bottom: 24px;
...
.clutch-title {
  font-family:'Bebas Neue',sans-serif; font-size:4rem; letter-spacing:0.12em;
  background: linear-gradient(135deg,#FF6B35,#FFB800,#FF6B35); background-size:200% auto;
  -webkit-background-clip:text; -webkit-text-fill-color:transparent; background-clip:text;
  animation:shimmer 4s linear infinite; line-height:1; margin:0;
}
.clutch-sub    { font-family:'JetBrains Mono',monospace; font-size:.75rem; color:var(--text-muted); letter-spacing:.2em; text-transform:uppercase; margin-top:6px; }
.clutch-tagline{ font-family:'Inter',sans-serif; font-weight:300; font-size:.9rem; color:#aaaacc; margin-top:10px; }

.pred-card { background:var(--bg-card); border:1px solid var(--border); border-radius:12px; padding:20px 24px; margin-bottom:16px; animation:slideInUp .4s ease both; transition:border-color .3s,transform .2s; }
.pred-card:hover { border-color:var(--clutch-orange); transform:translateY(-2px); }
.pred-card.strong { border-color:rgba(255,184,0,0.4); background:linear-gradient(135deg,#111118,#1a1200); }
.pred-pick  { font-family:'Bebas Neue',sans-serif; font-size:2rem; letter-spacing:.05em; color:var(--clutch-orange); line-height:1; }
.pred-game  { font-family:'JetBrains Mono',monospace; font-size:.75rem; color:var(--text-muted); text-transform:uppercase; letter-spacing:.1em; margin-bottom:4px; }
.conf-badge { font-family:'Bebas Neue',sans-serif; font-size:2.2rem; letter-spacing:.05em; }
.conf-strong{ color:var(--clutch-gold); }
.conf-good  { color:var(--clutch-green); }
.conf-lean  { color:var(--clutch-blue); }

.badge-won     { display:inline-block; background:rgba(0,230,118,.12); color:var(--clutch-green); border:1px solid var(--clutch-green); border-radius:6px; padding:4px 14px; font-family:'Bebas Neue',sans-serif; font-size:1.1rem; letter-spacing:.12em; animation:glowPulse 2s ease-in-out infinite; }
.badge-lost    { display:inline-block; background:rgba(255,23,68,.1);  color:var(--clutch-red);   border:1px solid var(--clutch-red);   border-radius:6px; padding:4px 14px; font-family:'Bebas Neue',sans-serif; font-size:1.1rem; letter-spacing:.12em; animation:redGlow 2s ease-in-out infinite; }
.badge-pending { display:inline-block; background:rgba(0,176,255,.08); color:var(--clutch-blue);  border:1px solid rgba(0,176,255,.3); border-radius:6px; padding:4px 14px; font-family:'Bebas Neue',sans-serif; font-size:1.1rem; letter-spacing:.12em; }

.record-display { font-family:'Bebas Neue',sans-serif; font-size:3.5rem; letter-spacing:.08em; text-align:center; line-height:1; animation:fadeIn .6s ease; }
.record-w { color:var(--clutch-green); }
.record-d { color:var(--text-muted); margin:0 8px; }
.record-l { color:var(--clutch-red); }
.streak-badge { font-family:'JetBrains Mono',monospace; font-size:.85rem; font-weight:700; letter-spacing:.1em; text-transform:uppercase; padding:3px 10px; border-radius:4px; display:inline-block; margin-top:4px; }
.streak-w { background:rgba(0,230,118,.15); color:var(--clutch-green); }
.streak-l { background:rgba(255,23,68,.1);  color:var(--clutch-red); }

.live-dot   { display:inline-block; width:10px; height:10px; background:var(--clutch-red); border-radius:50%; animation:livePulse 1.2s ease-in-out infinite; margin-right:6px; vertical-align:middle; }
.live-label { font-family:'JetBrains Mono',monospace; font-size:.75rem; color:var(--clutch-red); letter-spacing:.12em; text-transform:uppercase; vertical-align:middle; }

.slate-card  { background:var(--bg-surface); border-radius:10px; padding:14px 18px; margin:8px 0; border-left:3px solid var(--clutch-orange); animation:slideInLeft .3s ease both; display:flex; justify-content:space-between; align-items:center; }
.slate-teams { font-family:'Inter',sans-serif; font-size:.95rem; font-weight:600; color:var(--text-primary); }
.slate-time  { font-family:'JetBrains Mono',monospace; font-size:.8rem; color:var(--clutch-orange); letter-spacing:.05em; }

.section-title { font-family:'Bebas Neue',sans-serif; font-size:1.6rem; letter-spacing:.1em; color:var(--clutch-orange); border-bottom:1px solid var(--border); padding-bottom:8px; margin-bottom:16px; }

.score-card { background:var(--bg-card); border:1px solid var(--border); border-radius:12px; padding:18px 24px; margin-bottom:12px; transition:border-color .3s; }
.score-card:hover { border-color:rgba(255,107,53,.4); }
.score-num  { font-family:'Bebas Neue',sans-serif; font-size:2.8rem; letter-spacing:.05em; line-height:1; }
.score-team { font-family:'Inter',sans-serif; font-weight:600; font-size:.9rem; color:var(--text-muted); text-transform:uppercase; letter-spacing:.08em; }

[data-testid="stMetric"]      { background:var(--bg-card); border:1px solid var(--border); border-radius:10px; padding:16px !important; }
[data-testid="stMetricValue"] { font-family:'Bebas Neue',sans-serif !important; font-size:2rem !important; letter-spacing:.05em !important; }
[data-testid="stTabs"] button { font-family:'Inter',sans-serif !important; font-weight:600 !important; letter-spacing:.05em !important; }
[data-testid="stTabs"] button[aria-selected="true"] { color:var(--clutch-orange) !important; border-bottom-color:var(--clutch-orange) !important; }
[data-testid="stSidebar"] { background:var(--bg-dark) !important; }
...
.data-pill { display:inline-block; background:rgba(0,176,255,.1); color:var(--clutch-blue); border:1px solid rgba(0,176,255,.25); border-radius:20px; padding:2px 10px; font-family:'JetBrains Mono',monospace; font-size:.65rem; letter-spacing:.1em; text-transform:uppercase; margin:2px; vertical-align:middle; }
.ai-label  { font-family:'JetBrains Mono',monospace; font-size:.7rem; color:var(--clutch-gold); letter-spacing:.15em; text-transform:uppercase; opacity:.8; }

.persist-box { background:rgba(0,230,118,.05); border:1px solid rgba(0,230,118,.2); border-radius:10px; padding:14px 18px; margin:10px 0; }
</style>
...
def get_db():
    ...
    conn.execute('''CREATE TABLE IF NOT EXISTS win_loss_record (
        ...
        date TEXT, total_won INTEGER DEFAULT 0,
        total_lost INTEGER DEFAULT 0, win_rate REAL DEFAULT 0.0
    )''')
    ...
    conn.execute('''CREATE TABLE IF NOT EXISTS model_params (
        param_key TEXT PRIMARY KEY, value REAL
    )''')
...
def save_prediction(pred_dict: Dict, game_label: str):
    try:
        ...
        conn.execute('''INSERT OR IGNORE INTO predictions_log
            (id, game_id, game_label, bet_type, pick, line, confidence,
             factors, rationale, created_at)
            VALUES (?,?,?,?,?,?,?,?,?,?)''', (
...
def get_record_stats() -> Dict:
    try:
        ...
        return {
            ...
            'total_graded': won + lost, 'win_rate': win_rate,
...
# ══════════════════════════════════════════════════════════════
#  CLOUD PERSISTENCE — Survive Streamlit Cloud restarts
# ══════════════════════════════════════════════════════════════

def load_weights_from_secrets():
    """
    On startup, import learned weights + model params from Streamlit secrets
    if they exist. This lets the model survive Cloud restarts/redeploys.

    In App Settings → Secrets add:
        LEARNED_WEIGHTS = '{"Home court Q1": 1.35, ...}'
        MODEL_PARAMS    = '{"confidence_scalar": 1.82, "min_confidence": 52.5}'
    """
    if st.session_state.get('_secrets_loaded'):
        return
    st.session_state['_secrets_loaded'] = True
    try:
        weights_json = st.secrets.get("LEARNED_WEIGHTS", "")
        params_json  = st.secrets.get("MODEL_PARAMS", "")
        conn = get_db()
        loaded_w, loaded_p = 0, 0
        if weights_json:
            ...
            for name, w in weights.items():
                conn.execute(
                    '''INSERT OR REPLACE INTO factor_weights
                       ...
                       VALUES (?,?,COALESCE((SELECT update_count FROM factor_weights WHERE factor_name=?),0),?)''',
                    (name, float(w), name, utc_now().isoformat())
                )
            loaded_w = len(weights)
        if params_json:
            params = json.loads(params_json)
            for k, v in params.items():
                conn.execute("INSERT OR REPLACE INTO model_params VALUES (?,?)", (k, float(v)))
            loaded_p = len(params)
        if loaded_w or loaded_p:
            conn.commit()
    except Exception:
        pass


def export_weights_json() -> Tuple[str, str]:
    """Return (weights_json, params_json) for pasting into Streamlit secrets."""
    try:
        conn = get_db()
        rows = conn.execute("SELECT factor_name, weight FROM factor_weights").fetchall()
        weights = {r[0]: round(r[1], 6) for r in rows}
        prows   = conn.execute("SELECT param_key, value FROM model_params").fetchall()
        params  = {r[0]: round(r[1], 6) for r in prows}
        return json.dumps(weights), json.dumps(params)
    except Exception:
        return '{}', '{}'
...
def _calibrate_model(conn) -> None:
    """Adjust confidence scalar so predicted confidence ≈ actual win-rate."""
    try:
        rows = conn.execute(
        ...
        new_scalar = round(max(1.0, min(3.5, current_scalar - cal_err * 0.003)), 4)
        conn.execute(
...
def adapt_weights(factors_json: str, result: str) -> None:
    ...
    WON  + bullish factor  → increase weight
    LOST + bullish factor  → decrease weight
    Weights clamped [0.3, 6.0].
    """
        ...
        for fac in factors:
            ...
            conn.execute(
                '''INSERT OR REPLACE INTO factor_weights
                   (factor_name, weight, update_count, last_updated)
                   VALUES (?,?,?,?)''',
                (name, new_w, n_upd, utc_now().isoformat())
            )
    ...
    try:
        if bet_type == 'Moneyline':
            ...
            if line is None or actual == 0:
                return None
            correct = (actual > line) == ('OVER' in pick.upper())

        elif 'Total' in bet_type:
            if line is None or total == 0:
                return None
            correct = (total > line) == ('OVER' in pick.upper())
...
def grade_predictions() -> int:
    ...
    pending = conn.execute(
        "SELECT id, game_id, game_label, bet_type, pick, line "
        "FROM predictions_log WHERE result = 'pending'"
    ).fetchall()
    if not pending:
        return 0
    ...
    for pred_id, game_id, game_label, bet_type, pick, line in pending:
        ...
        if result:
            ...
            conn.execute(
                "UPDATE predictions_log SET result=?, final_score=?, "
                "rationale=rationale||? WHERE id=?",
...
def fetch_upcoming_events() -> List[Dict]:
    ...
    for delta in [0, 1, 2]:
        ...
        data = safe_get(f"{ESPN_BASE}/scoreboard", HEADERS_ESPN,
...
def fetch_team_stats(team_id: str) -> Dict:
    if not team_id:
        return {}
    key = f"tstats_{team_id}_{utc_now().strftime('%Y%m%d')}"
...
def fetch_injuries() -> List[Dict]:
    key = f"injuries_{utc_now().strftime('%Y%m%d%H')}"
...
def fetch_team_schedule(team_id: str) -> List[Dict]:
    if not team_id:
        return []
    key = f"sched_{team_id}_{utc_now().strftime('%Y%m%d')}"
    cached = db_get(key, ttl=3600)
    if cached:
        return cached
    ...
    for ev in data.get('events', []):
        ...
        games.append({
            ...
            'q1_home': _q(home, 1), 'q1_away': _q(away, 1),
            'q2_home': _q(home, 2), 'q2_away': _q(away, 2),
            'q3_home': _q(home, 3), 'q3_away': _q(away, 3),
            'status':     status_name,
        })
    db_set(key, games)
    return games
...
def fetch_nba_advanced_stats() -> Dict[str, Dict]:
    key = f"adv_stats_{utc_now().strftime('%Y%m%d')}"
    cached = db_get(key, ttl=86400)
    if cached:
        return cached
    data = safe_get(
        f"{NBA_STATS}/leaguedashteamstats", HEADERS_NBA,
        params={
            'Season': SEASON, 'SeasonType': 'Regular Season',
            'PerMode': 'PerGame', 'MeasureType': 'Advanced',
            'PaceAdjust': 'N', 'LeagueID': '00',
        }, timeout=20
    )
    result = {}
    if data:
...
def fetch_nba_base_stats() -> Dict[str, Dict]:
    key = f"base_stats_{utc_now().strftime('%Y%m%d')}"
    cached = db_get(key, ttl=86400)
    if cached:
        return cached
    data = safe_get(
...
class Factors:
    ...
    def home_court(is_home: bool) -> Tuple[float, str]:
        return (65.0, "Home court advantage (+3 pts)") if is_home else (38.0, "Road disadvantage")

    @staticmethod
    def offense_from_log(games: List[Dict], team_name: str) -> Tuple[float, str]:
        scores = [
            float(g.get('home_score' if g.get('home_team','') == team_name else 'away_score', 0))
            for g in games
            if g.get('status') in STATUS_FINAL
    ...
    def defense_from_log(games: List[Dict], team_name: str) -> Tuple[float, str]:
        allowed = [
            float(g.get('away_score' if g.get('home_team','') == team_name else 'home_score', 0))
            for g in games
            if g.get('status') in STATUS_FINAL
    ...
    def advanced_ortg(adv: Dict, team_abbr: str) -> Tuple[float, str]:
        ...
        return min(100, max(0, (ortg - 100) / 20 * 100)), f"ORTG: {ortg:.1f}"

    @staticmethod
    def advanced_drtg(adv: Dict, team_abbr: str) -> Tuple[float, str]:
        d    = adv.get(team_abbr, {})
        drtg = d.get('drtg', 113.0)
        return min(100, max(0, (125 - drtg) / 25 * 100)), f"DRTG: {drtg:.1f} (lower=better)"

    @staticmethod
    def advanced_pace(adv: Dict, team_abbr: str) -> Tuple[float, str]:
        d    = adv.get(team_abbr, {})
        pace = d.get('pace', 99.0)
        return min(100, max(0, (pace - 90) / 20 * 100)), f"Pace: {pace:.1f} poss/game"
    ...
    def win_pct(base: Dict, team_abbr: str) -> Tuple[float, str]:
        ...
        gp   = d.get('gp', 1)
        return wpct * 100, f"Season: {wpct:.1%} over {gp} games"
    ...
    def rest(team_games: List[Dict]) -> Tuple[float, str]:
        ...
        try:
            ...
            days    = (utc_now().replace(tzinfo=None) - last_dt.replace(tzinfo=None)).days
            if days == 0: return 25.0, "Back-to-back — fatigue risk"
            if days == 1: return 55.0, "1 day rest"
            if days >= 3: return 80.0, f"{days}d rest — fresh legs"
    ...
    def q1_avg(games: List[Dict], team_name: str) -> Tuple[float, str]:
        q1s = [
            float(g.get('q1_home' if g.get('home_team','') == team_name else 'q1_away', 0))
            for g in games
            if g.get('status') in STATUS_FINAL
    ...
    def b2b_check(games: List[Dict]) -> Tuple[bool, str]:
        completed = sorted([g for g in games if g.get('date')],
                           key=lambda x: x.get('date', ''))
        if len(completed) < 2:
            return False, "Not on back-to-back"
        try:
            last_dt   = datetime.fromisoformat(completed[-1]['date'].replace('Z', '+00:00'))
            days_since = (utc_now().replace(tzinfo=None) - last_dt.replace(tzinfo=None)).days
...
class Engine:
    def __init__(self, injury_map: Dict, adv_stats: Dict = None,
                 base_stats: Dict = None, learned_weights: Dict = None,
    ...
    def q1_spread(self, home: Dict, away: Dict, hg: List, ag: List,
        ...
        hw = self.W("Home Q1 scoring") + self.W("Home starters") + self.W("Home court Q1")
        home_edge = (hq_s * self.W("Home Q1 scoring") + hi_s * self.W("Home starters") +
                     hca_s * self.W("Home court Q1")) / hw
        aw = self.W("Away Q1 scoring") + self.W("Away absences")
        away_edge = (aq_s * self.W("Away Q1 scoring") + ai_s * self.W("Away absences")) / aw

        pick_team   = home['name'] if home_edge >= away_edge else away['name']
        margin_est  = round(abs(home_edge - away_edge) / 5, 1)
    ...
    def q1_total(self, home: Dict, away: Dict, hg: List, ag: List,
        ...
        f.append(Factor("Combined Q1 pace", self.W("Combined Q1 pace"),
                         (hq_s + aq_s) / 2, "Both teams Q1 tendency"))

        hi_s, _ = Factors.health(home['id'], self.inj)
        ai_s, _ = Factors.health(away['id'], self.inj)
        f.append(Factor("Combined health",  self.W("Combined health"),
                         (hi_s + ai_s) / 2, "H+A roster health"))

        if h_abbr and a_abbr:
            h_pace, _ = Factors.advanced_pace(self.adv, h_abbr)
            a_pace, _ = Factors.advanced_pace(self.adv, a_abbr)
            f.append(Factor("Advanced pace", self.W("Advanced pace"),
                             (h_pace + a_pace) / 2,
                             f"H:{self.adv.get(h_abbr,{}).get('pace',99):.1f} "
                             f"A:{self.adv.get(a_abbr,{}).get('pace',99):.1f}"))

        b2b_h, _ = Factors.b2b_check(hg)
        b2b_a, _ = Factors.b2b_check(ag)
        if b2b_h or b2b_a:
            f.append(Factor("B2B sluggish start", self.W("B2B sluggish start"),
                             20.0, "B2B → Q1 typically 2-4 pts below avg"))
...
# ══════════════════════════════════════════════════════════════
#  COUNTDOWN COMPONENT
# NOTE: st.components.v1.html is deprecated (removal June 2026).
# st.html() cannot run <script> tags, so we keep it for now.
# Countdowns are deduplicated via session_state to limit spam.
# ══════════════════════════════════════════════════════════════
...
def render_countdown(tip_off_utc: str, game_label: str):
    ...
    # Deduplicate: only render each countdown once per session
    seen_key = f"_cd_{uid}"
...
<div style="display:flex;align-items:center;gap:10px;margin:4px 0;">
  <span style="font-family:'JetBrains Mono',monospace;font-size:.65rem;
               color:#666;letter-spacing:.12em;text-transform:uppercase;">TIP-OFF IN</span>
  <span id="cd_{uid}" style="font-family:'JetBrains Mono',monospace;font-size:1.3rem;
              font-weight:700;color:#FF6B35;letter-spacing:.05em;">--:--:--</span>
  <span style="font-family:'JetBrains Mono',monospace;font-size:.6rem;color:#555;">CAT</span>
</div>
<script>
(function(){{
  var tip={epoch_ms}, el=document.getElementById('cd_{uid}');
  function fmt(n){{return n<10?'0'+n:''+n;}}
  function tick(){{
    var d=tip-Date.now();
    if(d<=0){{el.textContent='🏀 LIVE';el.style.color='#FF1744';return;}}
    var h=Math.floor(d/3600000),m=Math.floor((d%3600000)/60000),s=Math.floor((d%60000)/1000);
    el.textContent=fmt(h)+':'+fmt(m)+':'+fmt(s);
    if(d<1800000)el.style.color='#FFB800';
    if(d<300000) el.style.color='#FF1744';
...
def render_pred(pred: Prediction):
    ...
    card_cls  = "pred-card strong" if is_strong else "pred-card"
...
<div class="{card_cls}">
  ...
  <div style="font-family:'Inter',sans-serif;font-size:.82rem;color:#aaa;margin-top:6px;">
...
<div style="text-align:center;padding:12px 0;">
  ...
  <div style="font-family:'JetBrains Mono',monospace;font-size:.7rem;
              color:#888;letter-spacing:.1em;margin-top:4px;">{tier}</div>
  <div style="font-family:'Inter',sans-serif;font-size:.75rem;color:#666;margin-top:4px;">
...
def render_results_tab(tab):
    with tab:
        ...
        with col_refresh:
            if st.button("🔄 Refresh & Grade", width='stretch'):
                st.rerun()
        ...
        try:
            ...
            df_all = pd.DataFrame(all_rows, columns=cols) if all_rows \
        ...
        streak_css   = ('streak-w' if streak_char == 'WON'
                        else ('streak-l' if streak_char == 'LOST' else ''))

        st.markdown(f"""
<div style="background:#111118;border:1px solid rgba(255,107,53,.15);
            border-radius:14px;padding:28px;text-align:center;margin-bottom:20px;">
  <div style="font-family:'JetBrains Mono',monospace;font-size:.7rem;color:#666;
              letter-spacing:.2em;text-transform:uppercase;margin-bottom:8px;">All-Time Record</div>
  ...
  <div style="font-family:'Bebas Neue',sans-serif;font-size:1.6rem;
              ...
              letter-spacing:.1em;margin-top:6px;">{win_rate:.1f}% WIN RATE</div>
  <div style="margin-top:10px;">
    <span class="streak-badge {streak_css}">STREAK: {streak_label}</span>
    &nbsp;
    <span class="badge-pending" style="font-size:.8rem;">{len(pending_df)} PENDING</span>
  </div>
</div>""", unsafe_allow_html=True)
        ...
        if total_graded > 0:
            ...
            for bt in sorted(graded_df['bet_type'].unique()):
                ...
                bt_stats.append({'Bet Type': bt, '✅ Won': w, '❌ Lost': l,
                                  'Total': len(bt_df), 'Win %': f"{w/len(bt_df)*100:.0f}%"})
            st.dataframe(pd.DataFrame(bt_stats), hide_index=True, width="stretch")
            st.divider()
        ...
        if total_graded >= 3:
            ...
            for r in last10:
                if r == 'WON':
                    dots += '<span class="badge-won" style="font-size:.75rem;padding:2px 8px;margin:2px;">W</span>'
                else:
                    dots += '<span class="badge-lost" style="font-size:.75rem;padding:2px 8px;margin:2px;">L</span>'
            st.markdown(dots, unsafe_allow_html=True)
            st.divider()

        st.markdown(f"### ✅ Correct ({len(won_df)})")
        if won_df.empty:
            st.info("No wins graded yet — results appear after games finish.")
        else:
            for _, r in won_df.iterrows():
                score_str = str(r.get('final_score','') or '').strip()
                st.markdown(f"""
<div style="background:rgba(0,230,118,.05);border:1px solid rgba(0,230,118,.2);
            border-radius:10px;padding:14px 18px;margin:8px 0;">
  <div style="display:flex;justify-content:space-between;align-items:flex-start;">
    <div>
      <div style="font-family:'Inter',sans-serif;font-weight:700;font-size:.95rem;
                  color:#F0F0F5;">{r['bet_type']} · {r['game_label']}</div>
      <div style="font-family:'Bebas Neue',sans-serif;font-size:1.4rem;
                  color:#FF6B35;letter-spacing:.05em;">{r['pick']}</div>
      <div style="font-size:.78rem;color:#666;margin-top:2px;">
        {str(r.get('rationale',''))[:110]}…
        {f'<br><span style="color:#4ade80;font-size:.75rem;">📊 {score_str}</span>' if score_str else ''}
      </div>
    </div>
    <div style="text-align:right;flex-shrink:0;padding-left:16px;">
      ...
      <div style="font-size:.7rem;color:#666;">
        ...
        if lost_df.empty:
            st.info("No losses graded yet.")
        else:
            for _, r in lost_df.iterrows():
                score_str = str(r.get('final_score','') or '').strip()
                st.markdown(f"""
<div style="background:rgba(255,23,68,.04);border:1px solid rgba(255,23,68,.18);
            border-radius:10px;padding:14px 18px;margin:8px 0;">
  <div style="display:flex;justify-content:space-between;align-items:flex-start;">
    <div>
      <div style="font-family:'Inter',sans-serif;font-weight:700;font-size:.95rem;
                  color:#F0F0F5;">{r['bet_type']} · {r['game_label']}</div>
      <div style="font-family:'Bebas Neue',sans-serif;font-size:1.4rem;
                  color:#888;letter-spacing:.05em;">{r['pick']}</div>
      <div style="font-size:.78rem;color:#666;margin-top:2px;">
        {str(r.get('rationale',''))[:110]}…
        {f'<br><span style="color:#f87171;font-size:.75rem;">📊 {score_str}</span>' if score_str else ''}
      </div>
    </div>
    <div style="text-align:right;flex-shrink:0;padding-left:16px;">
      ...
      <div style="font-size:.7rem;color:#666;">
        ...
        if not pending_df.empty:
            ...
            with st.expander(f"⏳ Pending — {len(pending_df)} awaiting results"):
                ...
                st.dataframe(pd.DataFrame(pend_data), hide_index=True,
                             width='stretch')
...
def render_learning_tab(tab):
    with tab:
        st.markdown('<div class="section-title">🧠 SELF-LEARNING MODEL</div>',
                    unsafe_allow_html=True)
        st.markdown("""
<div style="background:#111118;border:1px solid rgba(255,184,0,.2);border-radius:12px;
            padding:16px 20px;margin-bottom:20px;font-family:'Inter',sans-serif;
            font-size:.85rem;color:#aaa;">
  <b style="color:#FFB800;">How it works:</b>
  Every time a prediction is graded (WON/LOST), the engine performs an EMA weight update
  on every factor that contributed to that prediction. Factors that were bullish and correct
  get stronger; bullish but wrong get weaker. The confidence scalar is also calibrated so
  predicted confidence tracks actual win-rate. All changes persist in SQLite and — when
  you export to Streamlit Secrets — survive Cloud restarts.
</div>""", unsafe_allow_html=True)
        ...
        # ── Cloud Persistence Export ──────────────────────────
        st.markdown("#### 🔒 Cloud Persistence — Save Weights to Secrets")
        st.markdown("""
<div class="persist-box">
  <b style="color:#00E676;">How to make the model remember across restarts:</b><br>
  1. Click <b>Generate Export</b> below<br>
  2. Copy each JSON block<br>
  3. Go to your Streamlit Cloud app → <b>Settings → Secrets</b><br>
  4. Paste as:<br>
  <code>LEARNED_WEIGHTS = '{"Home court Q1": 1.35, ...}'</code><br>
  <code>MODEL_PARAMS = '{"confidence_scalar": 1.82, "min_confidence": 52.5}'</code><br>
  5. Save → the model reloads these on every restart automatically ✅
</div>""", unsafe_allow_html=True)

        if st.button("📤 Generate Export JSON", type="primary"):
            w_json, p_json = export_weights_json()
            st.markdown("**Paste into Secrets as `LEARNED_WEIGHTS`:**")
            st.code(w_json, language='json')
            st.markdown("**Paste into Secrets as `MODEL_PARAMS`:**")
            st.code(p_json, language='json')
            if w_json == '{}':
                st.warning("No learned weights yet — grade some predictions first.")
        ...
        try:
            rows = get_db().execute(
                "SELECT factor_name, weight, update_count, last_updated "
                "FROM factor_weights ORDER BY weight DESC"
            ).fetchall()
        except Exception:
            rows = []
        ...
        for name in DEFAULT_WEIGHTS:
            ...
            if name in learned_map:
                ...
                status = ("📈 Stronger" if lw > default_w * 1.05
                          else ("📉 Weaker" if lw < default_w * 0.95 else "≈ Stable"))
            else:
        ...
        if table_data:
            ...
            st.dataframe(df_w.style.map(_color_change, subset=['Change']),
                         hide_index=True, width="stretch")
        else:
            st.info("No weights learned yet — grade predictions to populate this table.")
        ...
        try:
            ...
            if len(hist_rows) >= 3:
                ...
                cal_df['rolling_wr']  = cal_df['correct'].rolling(5, min_periods=1).mean()
                cal_df['rolling_conf']= cal_df['confidence'].rolling(5, min_periods=1).mean()
        ...
        with st.expander("⚠️ Reset learned weights to defaults"):
            st.warning("This wipes all learned weights. Use only if the model is misbehaving.")
            if st.button("🔄 Reset All Learned Weights", type="primary"):
                try:
                    ...
                    conn.commit()
                    st.success("✅ Reset complete. Reload the page to apply.")
                except Exception as e:
...
def main():
    ...
    # ── Load persisted weights from secrets (once per session) ─
    load_weights_from_secrets()
...
<div style="text-align:center;">
  <div style="font-family:'Bebas Neue',sans-serif;font-size:.8rem;
              letter-spacing:.15em;color:#666;">RECORD</div>
  <div style="font-family:'Bebas Neue',sans-serif;font-size:2.5rem;letter-spacing:.05em;">
    <span style="color:#00E676;">{rec['won']}</span>
    <span style="color:#444;">–</span>
    <span style="color:#FF1744;">{rec['lost']}</span>
  </div>
  <div style="font-family:'JetBrains Mono',monospace;font-size:.75rem;color:#888;">
    {rec['win_rate']:.1f}% · Streak: {rec['streak']}
  </div>
</div>""", unsafe_allow_html=True)
        st.divider()
        st.caption(f"🕐 {utc_now().strftime('%H:%M:%S UTC')}")
        st.caption("💡 Add ODDS_API_KEY in Streamlit Secrets")
        st.caption("💾 Add LEARNED_WEIGHTS + MODEL_PARAMS to persist learning")
...
<div class="clutch-hero">
  <div style="display:flex;justify-content:space-between;align-items:flex-start;
    ...
    <div>
      ...
      <div class="clutch-tagline">
        Self-learning AI predictions — 10+ data sources · Live injury data · Full transparency
      </div>
    </div>
    <div style="text-align:right;">
      <div style="font-family:'Bebas Neue',sans-serif;font-size:.75rem;
                  letter-spacing:.2em;color:#555;margin-bottom:4px;">ALL-TIME</div>
  ...
  <div style="margin-top:14px;display:flex;gap:8px;flex-wrap:wrap;">
    ...
    <span class="data-pill" style="background:rgba(255,184,0,.12);color:#FFB800;
           border-color:rgba(255,184,0,.3);">🧠 Self-Learning</span>
    <span class="data-pill" style="background:rgba(0,230,118,.1);color:#00E676;
           border-color:rgba(0,230,118,.3);">💾 Cloud Persistent</span>
  </div>
</div>""", unsafe_allow_html=True)
    ...
    # ── Tabs ──────────────────────────────────────────────────
    tab_preds, tab_live, tab_injuries, tab_teams, \
        tab_history, tab_results, tab_learn = st.tabs([
        "🎯 Predictions", "🟢 Live", "🏥 Injuries",
        "📊 Teams", "📖 Log", "🏆 Results", "🧠 Learning"
    ])

    engine = Engine(
        inj_map, adv_stats, base_stats,
    ...
    # ══════════════════════════════════════════════════════════
    with tab_preds:
        st.markdown('<div class="section-title">TODAY\'S PICKS</div>',
                    unsafe_allow_html=True)
        ...
        if not upcoming_events:
            ...
            with st.expander(f"📅 {len(upcoming_events)} game(s) on the slate",
                             expanded=True):
...
<div class="slate-card">
  <div>
    ...
    <div class="slate-time">🕐 {tip_cat}</div>
                    ...
                    render_countdown(tip_utc, f"slate_{a_name}@{h_name}")
            ...
            for idx, event in enumerate(upcoming_events):
                ...
                for key_frag, od in odds_map.items():
                    if (key_frag in home['name'].lower() or
                            home['name'].lower().split()[-1] in key_frag):
                        matched_odds = od
                        break
...
<div style="font-family:'JetBrains Mono',monospace;font-size:.7rem;color:#666;
  ...
  🔥 STRONG ≥75% · ✅ SOLID 65–74% · ⚡ LEAN &lt;65% · sorted by confidence
    ...
    # ══════════════════════════════════════════════════════════
    with tab_live:
        st.markdown(f'<div class="section-title">LIVE · '
                    f'{utc_now().strftime("%d %b %Y")}</div>',
                    unsafe_allow_html=True)
        if not games:
            ...
            for event in games:
                ...
                live_html = ('<span class="live-dot"></span>'
                             '<span class="live-label">LIVE</span>') if is_live else ''
...
<div class="score-card">
  <div style="display:flex;justify-content:space-between;align-items:center;
    ...
    <div style="text-align:center;padding:0 8px;">
      <div style="font-family:'Bebas Neue',sans-serif;font-size:1rem;color:#444;
                  letter-spacing:.1em;">VS</div>
  ...
  {"<div style='font-family:JetBrains Mono,monospace;font-size:.72rem;color:#666;margin-top:8px;'>" + odds_str + "</div>" if odds_str else ""}
</div>""", unsafe_allow_html=True)
    ...
    # ══════════════════════════════════════════════════════════
    with tab_injuries:
        st.markdown('<div class="section-title">INJURY REPORT</div>',
                    unsafe_allow_html=True)
        if not injuries:
            ...
            for team_name in sorted(teams_inj.keys()):
                ...
                with st.expander(f"🏀 {team_name}  ({len(inj_list)})"):
                    for inj in inj_list:
                        ...
                        ic, css = next(
                            ((v, c) for k, (v, c) in STATUS_ICONS.items() if k in s),
                            ('⚪', '')
                        )
                        pos = f"[{inj.get('position','')}]" if inj.get('position') else ""
                        st.markdown(
                            f"{ic} **{inj.get('player','?')}** {pos} — "
                            f"<span class='{css}'>{s}</span>",
                            unsafe_allow_html=True
                        )
    ...
    # ══════════════════════════════════════════════════════════
    with tab_teams:
        st.markdown('<div class="section-title">TEAM DEEP DIVE</div>',
                    unsafe_allow_html=True)
        ...
        try:
            ...
            if rows:
                df = pd.DataFrame(rows,
                                  columns=['Game','Bet Type','Pick','Confidence%',
                                           'Result','Logged'])
